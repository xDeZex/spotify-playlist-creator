...
Each OpenSpec task produces one commit, called `task X.Y: <short description>`. Don't use the commit skill. After each task commit, run the `code-review` skill, fix any findings in a single `fix: review finding — <short description>` commit, then move on. Fix commits at this stage do not trigger another review.
